...
interface TaskContextType {
    createTask: (groupId: string, task: Task) => Promise<void>;
...
export const TaskProvider = ({ children }: { children: ReactNode }) => {
    ...
    const createTask = async (groupId: string, task: Task) => {
        console.log(task)
        try {
            const res = await createTaskRequest(groupId, task);
    ...
    return (
        <TaskContext.Provider value={{ createTask, getTasks, getTask, deleteTask, updateTask, tasks, errorsTask }}>
            {children}
